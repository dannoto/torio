...
def add_persona( campanha_data, demanda, nome, username, email, telefone):

    url = base_url+"add_persona?persona_nome="+str(nome)+"&persona_username="+str(username)+"&persona_email="+str(email)+"&persona_telefone="+str(telefone)+"&persona_tag_id="+str(campanha_data['campanha_tag_id'])+"&oferta_agente_id=0&is_deleted=0"
...
def add_person_process(user_data, campanha_data, headers, demanda):

        # Links
                links = ""
                
                try:
                    for link in user_data['result']['user']['bio_links']:
                ...
                try:
                    for mencao in user_data['result']['user']['biography_with_entities']['entities']:
                ...
                persona = {
                        # 'tarefa_id': tarefa_id,
                        # 'tag_id': tag_id,
                        'username': user_data['result']['user']['username'],
                        'full_name': user_data['result']['user']['full_name'],
                        'is_private': user_data['result']['user']['is_private'],
                        'biografia': user_data['result']['user']['biography'],
                        'links': links,
                        'mencoes': mencoes,
                        # 'categoria': user_data['result']['user']['category_name'],
                        'email': extractEmail(None, user_data['result']['user']['biography'], links, headers, mencoes),
                ...
                add_persona(
                    campanha_data, 
                    demanda,
                    user_data['result']['user']['full_name'], 
                    user_data['result']['user']['username'],
                    extractEmail(None, user_data['result']['user']['biography'], links, headers, mencoes),
                    extractTelefone(None, links, user_data['result']['user']['biography'], mencoes, headers)
                )
    ...
    if campanha_data:
        ...
        if demandas_pendentes and len(demandas_pendentes) > 0:
            ...
            for demanda in demandas_pendentes:
                    ...
                    url = 'https://gramsnap.com/api/ig/userInfoByUsername/'+demanda['username']
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            add_person_process(user_data, campanha_data, header, demanda)
